...
#include <Gstreamer/gst/video/videooverlay.h>
...
static void playbinNotifySource (GObject *o, GstMessage *msg, GStreamerBackend *self) {
    GstElement *source;
...
static void check_media_size (GStreamerBackend *self) {
    ...
    if (!gst_video_info_from_caps(&info, caps)){
        ...
        width = info.width;
        height = info.height;
        GST_DEBUG_OBJECT (video_sink, "video pixel-aspect-ratio %d/%d", info.par_n, info.par_d);
...
-(void) app_function
{
    ...
    g_signal_connect (G_OBJECT (pipeline), "source-setup", (GCallback)playbinNotifySource,  (__bridge void *)self);
