...
static gboolean refresh_ui (CustomData *data) {
  ...
  if (!GST_CLOCK_TIME_IS_VALID (data->duration)) {
    if (!gst_element_query_duration (data->pipeline, fmt, &data->duration)) // no longer &fmt!
  ...
  if (gst_element_query_position (data->pipeline, fmt, &position))  // no longer &fmt!
     {
...
static void *app_function (void *userdata) {
  ...
  /* Build pipeline */
  data->pipeline = gst_parse_launch("playbin", &error); // playbin2
  if (error) {
  ...
  GST_DEBUG ("Pipeline Created..... Entering main loop... (CustomData:%p)", data);
