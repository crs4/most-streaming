...
class GStreamerBackend implements SurfaceHolder.Callback, IStream {
    ...
    private void initStream(SurfaceView surface, boolean frameCallback) {
        ...
        if (surface == null) {
            ...
            this.notifyState(new StreamingEventBundle(StreamingEventType.STREAM_EVENT,
                StreamingEvent.STREAM_ERROR, "No valid surface provided for the stream: " + this.streamName, this));
            return;
        }
        else {
...
//			surfaceInit(surface.getHolder().getSurface());
        ...
        this.notifyState(new StreamingEventBundle(StreamingEventType.STREAM_EVENT,
            StreamingEvent.STREAM_STATE_CHANGED, "Inizializating Stream " + this.streamName, this));

        boolean native_init_result = nativeInit(this.streamName, this.latency, frameCallback);
        if (!native_init_result) {
            this.streamState = StreamState.DEINITIALIZED;
            this.notifyState(new StreamingEventBundle(StreamingEventType.STREAM_EVENT,
                StreamingEvent.STREAM_ERROR, "Stremm initialization failed:" + this.streamName + " initialized", this));
        }
        else {
...
//		else
    ...
    private void onGStreamerInitialized() {
        ...
        if (!this.stream_initialized) {
            ...
            this.notifyState(new StreamingEventBundle(StreamingEventType.STREAM_EVENT,
                ...
                "Stream " + this.streamName + " initialized: (uri:" + this.uri + ")", this));
        }
        else {
    ...
    private void onMediaSizeChanged(int width, int height) {
        ...
        this.notifyState(new StreamingEventBundle(StreamingEventType.STREAM_EVENT,
            StreamingEvent.VIDEO_SIZE_CHANGED, "Stream state changed to:" + this.streamState, this));
    }
    ...
    /**
     * Get the current value of latency property of this stream (Reads the value from native code to be sure to
     * return the effective latency value)
     *
