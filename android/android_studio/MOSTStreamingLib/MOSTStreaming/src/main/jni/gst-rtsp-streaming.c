...
static void media_configure(GstRTSPMediaFactory *factory, GstRTSPMedia *media, CustomData *data){
    ...
    g_object_set_data_full (G_OBJECT (media), "custom-data-ctx", data->context, (GDestroyNotify) g_free);
...
static void *app_function (void *userdata) {
    ...
    gst_rtsp_server_set_service(server, DEFAULT_RTSP_PORT);
    ...
    // g_object_set(factory, "stop-on-disconnect", TRUE);
    ...
    gst_rtsp_media_factory_set_shared (factory, NULL);
    ...
    GstStateChangeReturn res = gst_element_set_state (data->pipeline, GST_STATE_NULL);
    GST_ERROR("Element state result is %d ", res);
...
static void gst_native_init (JNIEnv* env, jobject thiz) {
    CustomData *data = g_new0 (CustomData, 1);  // g_new0 is a g_object func. It allocates 1 objects of type CustomData and init it to 0's
    ...
    gst_debug_set_threshold_for_name("most-streaming-rtsp", GST_LEVEL_DEBUG);
